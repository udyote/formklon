...
<input type=\"radio\" name=\"{{ q.entry_id }}\" value=\"_other_option_\" {% if q.required %}required{% endif %}>
<span>Diğer:</span>
...
{% if q.has_other %}
<label class=\"other-option-label\">
<input type=\"checkbox\" name=\"{{ q.entry_id }}\" value=\"_other_option_\">
<span>Diğer:</span>
<input type=\"text\" class=\"other-option-input\" name=\"{{ q.entry_id }}.other_option_response\">
</label>{% endif %}
</div>
...
def submit():
    ...
    for question in form_structure['questions']:
        ...
        if q_type == 'Onay Kutuları':
            ...
            final = []
            if "_other_option_" in answers:
                answers.remove("_other_option_")
